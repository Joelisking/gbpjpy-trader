...
//|                                                                  |
//|  Protocol:                                                       |
//|    EA writes  → MQL5/Files/ai_request.json                      |
//|    Python reads, deletes request, writes ai_response.json       |
//|    EA polls   → MQL5/Files/ai_response.json (up to timeoutMs)  |
//|    EA reads + deletes response                                   |
//+------------------------------------------------------------------+
#ifndef SCALPER_FILEAICLIENT_MQH
#define SCALPER_FILEAICLIENT_MQH
...
private:
    ...
    bool WriteRequest(const string &json)
    {
        ...
        if(h == INVALID_HANDLE)
        {
            PrintFormat("[FileAIClient] Cannot write request file (error %d)", GetLastError());
            return false;
        }
...
public:
    CFileAIClient(int timeoutMs = 500)
        : m_timeoutMs(timeoutMs),
          m_requestFile("ai_request.json"),
          m_responseFile("ai_response.json")
    {}
    ...
    SAIResponse RequestScoreSafe(const string &featuresJson, const string &direction)
    {
        ...
        string request = StringFormat(
            "{\"type\":\"entry_check\",\"symbol\":\"%s\","
            "\"direction\":\"%s\",\"tf\":\"M1\",\"features\":%s}",
        ...
        if(StringLen(json) == 0)
        {
            // Python never responded — clean up stale request
            if(FileIsExist(m_requestFile)) FileDelete(m_requestFile);
            PrintFormat("[FileAIClient] No response after %dms", m_timeoutMs);
            return resp;
        }
        ...
        if(!resp.valid)
            PrintFormat("[FileAIClient] Parse error. Raw: %s", StringSubstr(json, 0, 100));
...
#endif // SCALPER_FILEAICLIENT_MQH
